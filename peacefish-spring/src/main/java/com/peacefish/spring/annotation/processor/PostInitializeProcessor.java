...
public class PostInitializeProcessor implements SmartInitializingSingleton {
	 ...
	 * Internal logger
	 */
	private static final Logger logger = LoggerFactory.getLogger(PostInitializeProcessor.class);
	...
	public void afterSingletonsInstantiated() {
		...
		logger.info(format("Launched {0} method(s) in {1,number,#}ms", methodCount,
	...
	private static class PostInitializingMethod implements Callable<Void> {
		...
		public Void call() throws Exception {
			logger.debug(format("Initializing method {0} in class {1}", this.method.getName(),
	...
	public void postConstruct() {
		logger.info(format("Registered {0} with configuration : maxThread = [{1}]",
