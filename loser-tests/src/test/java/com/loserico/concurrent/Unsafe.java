...
/**
 * A collection of methods for performing low-level, unsafe operations. Although the class and
 * all methods are public, use of this class is limited because only trusted code can obtain
 * instances of it.
 *
 * @author John R. Rose
 * @see #getUnsafe
...
public final class Unsafe {
	 ...
	 *
	 * <p>
	 * The returned <code>Unsafe</code> object should be carefully guarded by the caller, since it
	 * can be used to read and write data at arbitrary memory addresses. It must never be passed to
	 * untrusted code.
	 *
	 * <p>
	 * Most methods in this class are very low-level, and correspond to a small number of hardware
	 * instructions (on typical machines). Compilers are encouraged to optimize these methods
	 * accordingly.
	 *
	 * <p>
	...
	/**
	 * Fetches a value from a given Java variable. More specifically, fetches a field or array
	 * element within the given object <code>o</code> at the given offset, or (if <code>o</code> is
	 * null) from the memory address whose numerical value is the given offset.
	 * <p>
	 * The results are undefined unless one of the following cases is true:
	 * <ul>
	 * <li>The offset was obtained from {@link #objectFieldOffset} on the
	 * {@link java.lang.reflect.Field} of some Java field and the object referred to by
	 * <code>o</code> is of a class compatible with that field's class.
	 *
	 * <li>The offset and object reference <code>o</code> (either null or non-null) were both
	 * obtained via {@link #staticFieldOffset} and {@link #staticFieldBase} (respectively) from the
	 ...
	 * </ul>
	 * <p>
	 * If one of the above cases is true, the call references a specific Java variable (field or
	 * array element). However, the results are undefined if that variable is not in fact of the
	 * type returned by this method.
	 * <p>
	 * This method refers to a variable by means of two parameters, and so it provides (in effect) a
	 * <em>double-register</em> addressing mode for Java variables. When the object reference is
	 ...
	 * <p>
	 * The first two parameters are interpreted exactly as with {@link #getInt(Object, long)} to
	 * refer to a specific Java variable (field or array element). The given value is stored into
	 * that variable.
	 * <p>
	 ...
	 * <p>
	 * Unless the reference <code>x</code> being stored is either null or matches the field type,
	...
	/**
	 * This method, like all others with 32-bit offsets, was native in a previous release but is now
	 * a wrapper which simply casts the offset to a long value. It provides backward compatibility
	 ...
	 *
	 * <p>
	 * If the native pointer is less than bits wide, it is extended as an unsigned number to a Java
	 * long. The pointer may be indexed by any given byte offset, simply by adding that offset (as a
	 * simple integer) to the long representing the pointer. The number of bytes actually read from
	 * the target address maybe determined by consulting {@link #addressSize}.
	 *
	 * @see #allocateMemory
	 */
	 ...
	 * in 'long' units. If the effective address and length are (resp.) even modulo or 2, the stores
	 ...
	 *
	 * <p>
	 * This method determines each block's base address by means of two parameters, and so it
	 * provides (in effect) a <em>double-register</em> addressing mode, as discussed in
	 * {@link #getInt(Object,long)}. When the object reference is null, the offset supplies an
	 * absolute base address.
	 *
	 * <p>
	 ...
	 * place in 'long' units. If the effective addresses and length are (resp.) even modulo or 2,
	 ...
	 * {@link #staticFieldOffset}, {@link #objectFieldOffset}, or {@link #arrayBaseOffset}.
	 */
	public static final int INVALID_FIELD_OFFSET = -1;

	/**
	 * Returns the offset of a field, truncated to bits. This method is implemented as follows:
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * </blockquote>
	 * 
	 * @deprecated As of 1.4.1, use {@link #staticFieldOffset} for static fields and
	 *             {@link #objectFieldOffset} for non-static fields.
	 */
	@Deprecated
	public int fieldOffset(Field f) {
		if (Modifier.isStatic(f.getModifiers()))
	...
	/**
	 * Returns the base address for accessing some static field in the given class. This method is
	 ...
	 * </blockquote>
	 * 
	 * @deprecated As of 1.4.1, use {@link #staticFieldBase(Field)} to obtain the base pertaining to
	 *             a specific {@link Field}. This method works only for JVMs which store all statics
	 ...
	 * use the most significant bits. However, JVM implementations which store static fields at
	 * absolute addresses can use long offsets and null base pointers to express the field locations
	 * in a form usable by {@link #getInt(Object,long)}. Therefore, code which will be ported to
	 * such JVMs on 64-bit platforms must preserve all bits of static field offsets.
	 * 
	 * @see #getInt(Object, long)
	 */
	 ...
	 * <p>
	 * As of 1.4.1, offsets for fields are represented as long values, although the Sun JVM does not
	 * use the most significant bits. It is hard to imagine a JVM technology which needs more than a
	 * few bits to encode an offset within a non-array object, However, for consistency with other
	 ...
	 * via methods like {@link #getInt(Object, long)}. This value may be null. This value may refer
	 * to an object which is a "cookie", not guaranteed to be a real Object, and it should not be
	 * used in any way except as argument to the get and put routines in this class.
	 */
	 ...
	 * will be either or 8. Note that the sizes of other primitive types (as stored in native memory
	...
	/**
	 * Tries to lock the object. Returns true or false to indicate whether the lock succeeded. If it
	 * did, the object must be unlocked via {@link #monitorExit}.
	 */
	 ...
	 * samples, representing averages over the last 1, 5, and minutes, respectively.
	 *
	 * @params loadavg an array of double of size nelems
	 * @params nelems the number of samples to be retrieved and must be to 3.
	 *
	 * @return the number of samples actually retrieved; or - * if the load average is unobtainable.
	 */
